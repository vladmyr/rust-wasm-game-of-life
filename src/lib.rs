...
impl Universe {
    ...
    // unoptimized performance-wise
    fn live_neighbor_count(&self, row: u32, column: u32) -> u8 {
        let mut count = 0;
        for delta_row in [self.height - 1, 0, self.height + 1].iter().cloned() {
            for delta_col in [self.width - 1, 0, self.width + 1].iter().cloned() {
...
impl Universe {
    ...
    pub fn new(width: u32, height: u32, preset: UniversePreset) -> Universe {
        utils::set_panic_hook();

        let cells = (0..width * height).map(|i| {
            match preset {
                UniversePreset::Empty => {
                    Cell::Dead
                },
                UniversePreset::Demo => {
                    if i % 2 == 0 || i % 7 == 0 {
